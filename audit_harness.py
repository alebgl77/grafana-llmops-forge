...
Portable (Linux/macOS/Windows) : chemins dérivés du fichier, artefacts en
répertoire temporaire système, E/S en UTF-8. Aucune dépendance externe.

    python3 audit_harness.py
"""
import json, os, re, subprocess, sys, tempfile

# Sortie console en UTF-8 : évite un crash cp1252 sous Windows sur les ✅/❌/→.
try:
...
TMP = tempfile.mkdtemp(prefix="llmops_forge_audit_")
...
def run_forge(cap, outdir, bps="auto", extra=()):
    cpath = os.path.join(TMP, f"{outdir}.json")
    with open(cpath, "w", encoding="utf-8") as f:
        json.dump(cap, f)
    d = os.path.join(TMP, outdir)
    r = subprocess.run([sys.executable, os.path.join(SC, "forge_dashboards.py"),
                        "--capability", cpath, "--blueprints", bps,
                        "--out-dir", d, "--with-alerts",
...
with open(os.path.join(d, "deploy_manifest.json"), encoding="utf-8") as fh:
    man = json.load(fh)
...
reg = load_registry(os.path.join(SK, "references", "model_registry.json"))
...
r = subprocess.run([sys.executable, os.path.join(SC, "visual_audit.py"), "--uids",
                    "llmops-x-1,llmops-y-2", "--dashboards", os.path.join(TMP, "nonexistent"),
                    "--list-only"], capture_output=True, text=True, encoding="utf-8")
check("uids mode OK", r.returncode == 0 and "llmops-x-1" in r.stdout, r.stderr[-200:])

# --------------------------------------------------------------------- 8. SKILL.md
print("\n[8] SKILL.md")
with open(os.path.join(SK, "SKILL.md"), encoding="utf-8") as fh:
    txt = fh.read()
...
missing = [f for f in set(refs) if not os.path.exists(os.path.join(SK, f))]
