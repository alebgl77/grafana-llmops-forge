...
def bp_gateway(ctx: Ctx) -> Board | None:
    ...
    if not q:
        return None
    b = Board(det_uid("ai-gateway-operations", scope=ctx.uid_scope), "AI · Gateway Operations",
...
def bp_adoption(ctx: Ctx) -> Board | None:
    ...
    if not q:
        return None
    b = Board(det_uid("ai-adoption", scope=ctx.uid_scope), "AI · Internal Adoption",
...
def bp_inference(ctx: Ctx) -> Board | None:
    ...
    if not qv and not gpu:
        return None
    b = Board(det_uid("ai-inference-selfhosted", scope=ctx.uid_scope), "AI · Self-hosted Inference",
...
def bp_quality(ctx: Ctx) -> Board | None:
    ...
    b = Board(det_uid("ai-quality-evals", scope=ctx.uid_scope), "AI · Quality & Evaluations",
...
def _rule(uid_name, title, prom_uid, expr, threshold, op, folder_uid,
          ...
          runbook="", uid_scope=None):
    ann = {"summary": summary}
    if runbook:
        ann["__dashboardUid__"] = ""
        ann["runbook_url"] = runbook
    return {"uid": det_uid(uid_name, "alr", uid_scope), "title": title, "orgID": org_id,
            ...
            "labels": {"severity": severity, "origin": "llmops-forge",
                       "llmops_rule_identity": alert_logical_identity(uid_name)},
            "annotations": ann,
            "data": [
...
def build_alerts(ctx: Ctx, folder_uid: str, daily_budget: float,
    ...
    if qv and qv.kv:
        alert_rules.append(_rule(
            ...
            "critical", "5m", "OK", org, uid_scope=ctx.uid_scope))
    qe = ctx.q.get("evals")
    if qe:
...
def selftest_capability() -> dict:
    prom = "prom-selftest"
    return {"org_id": 1,
            "instance": {"url": "http://selftest", "version": "13.0.0",
...
def _write_json_atomic(path: str, value: dict) -> None:
    """Écrit un manifeste complet ou conserve le précédent, jamais un JSON tronqué."""
    tmp = path + ".tmp"
...
def main() -> int:
    ...
    ap.add_argument("--uid-scope", default=None,
                    help="Scope explicite pour des UIDs distincts par dossier/tenant; "
                         "sans cette option les UIDs historiques sont inchangés")
    ap.add_argument("--org-id", type=int, default=None,
                    help="Override explicite de l'organisation Grafana (sinon /api/org "
                         "en déploiement, ou la capability map hors ligne)")
    ap.add_argument("--best-effort", action="store_true",
                    help="Retourner 0 malgré un déploiement partiel; le manifeste reste "
                         "partial/failed")
    ...
    try:
        if args.deploy and not args.dry_run:
            ...
            ctx.org_id = client.resolve_org(args.org_id)
        elif args.org_id is not None:
            ctx.org_id = args.org_id
    except (GrafanaError, SystemExit) as e:
        print(f"[fail] unable to resolve Grafana organization: {e}", file=sys.stderr)
        return 2
    if ctx.org_id is None or ctx.org_id <= 0:
        print("[fail] organization is unknown; re-run discovery or pass --org-id.",
              file=sys.stderr)
        return 2
    ctx.uid_scope = args.uid_scope
    ...
    operation = "deploy" if args.deploy and not args.dry_run else "generate"
    ...
    for name, board in boards:
        ...
        manifest["dashboards"].append(
            {"blueprint": name, "uid": board.d["uid"], "title": board.d["title"],
             "url": None, "status": "pending" if operation == "deploy" else "succeeded",
    ...
    if operation == "deploy":
        ...
        try:
            folder = client.ensure_folder(args.folder, folder_uid)
        except (GrafanaError, SystemExit) as e:
            ...
            manifest["errors"].append(
                {"resource_type": "folder", "identifier": folder_uid,
                 "operation": "ensure", "status": getattr(e, "status", 0),
                 "message": _perm_hint("folder", e)})
            manifest["deployment_status"] = "failed"
            ...
            # Un échec sur un dashboard n'annule pas les autres : le manifeste
            # décrit précisément l'état partiel, et le code retour le signale.
            for i, (name, board) in enumerate(boards):
                try:
                    res = client.upsert_dashboard(
                        board.d, actual_folder_uid,
                        f"llmops-forge {datetime.now(timezone.utc):%Y-%m-%d}")
                except (GrafanaError, SystemExit) as e:
                    hint = _perm_hint("dashboard", e)
                    manifest["resources"]["dashboards"]["failed"] += 1
                    manifest["dashboards"][i]["status"] = "failed"
                    manifest["dashboards"][i]["error"] = str(e)
                    manifest["errors"].append(
                        {"resource_type": "dashboard", "identifier": name,
                         "uid": board.d["uid"], "operation": "upsert",
                         "status": getattr(e, "status", 0), "message": hint})
                    print(f"  [fail] {name}: {hint}", file=sys.stderr)
                    continue
                manifest["resources"]["dashboards"]["succeeded"] += 1
                manifest["dashboards"][i]["status"] = "succeeded"
                manifest["dashboards"][i]["url"] = client.dashboard_url(res, board.d)
                print(f"  [ok] {manifest['dashboards'][i]['url']}")
            if manifest["resources"]["dashboards"]["failed"]:
                ds_stats = manifest["resources"]["dashboards"]
            ...
            for i, rule in enumerate(alert_rules):
                ...
                try:
                    ...
                    manifest["errors"].append(
                        {"resource_type": "alert", "identifier": rule["title"],
                         "uid": rule["uid"], "operation": "upsert",
                         "status": getattr(e, "status", 0),
                         "message": _perm_hint("alert", e)})
                    fb = os.path.join(args.out_dir, f"alert_{rule['uid']}.json")
                    with open(fb, "w", encoding="utf-8") as f:
                        json.dump(rule, f, indent=2, ensure_ascii=False)
                    print(f"  [warn] alert '{rule['title']}' not provisioned ({e}); "
                          f"export: {fb} (manual import possible)")
    elif args.with_alerts:
        for rule in alert_rules:
            fb = os.path.join(args.out_dir, f"alert_{rule['uid']}.json")
            with open(fb, "w", encoding="utf-8") as f:
                json.dump(rule, f, indent=2, ensure_ascii=False)
            print(f"[ok] alert (not deployed) -> {fb}")
    ...
    if operation == "deploy" and deploy_exit == 0:
        failed = sum(v["failed"] for v in manifest["resources"].values())
        succeeded = sum(v["succeeded"] for v in manifest["resources"].values())
        if failed:
            manifest["deployment_status"] = "partial" if succeeded else "failed"
            deploy_exit = 4
    _write_json_atomic(os.path.join(args.out_dir, "deploy_manifest.json"), manifest)

    if deploy_exit and not args.best_effort:
        return deploy_exit
